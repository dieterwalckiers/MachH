...
export const useEvents = routeLoader$(async (requestEvent: RequestEventLoader) => {
  ...
  const rawEvents = await sanityClient.fetch(`*[_type == "event" && date > now()]|order(date asc){date,time,place,price,title,slug,"image": image.asset->url,linkedProjects[]->{name, slug, hexColor}}[${fromMinusOne}...${toPlusOne}]`);
